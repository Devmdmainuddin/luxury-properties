...
import SingleWishlishItem from "../components/SingleWishlishItem";
import { Link } from "react-router-dom";
import { Helmet } from "react-helmet-async";

const WishlishItems = () => {
	const { Properties, wishlist, setwishlist } = useContext(PropertiesComponents)
	...
	return (
		...
		<div className="max-w-[1170px] mx-auto md:px-24 lg:px-8 flex flex-col  p-6 space-y-4 sm:p-10 bg-gray-900 text-gray-100">
			<Helmet>
             <title>Luxury Properties | wishlist </title>
             </Helmet>
			<h2 className="text-xl font-semibold">Your wishlist</h2>
			<ul className="flex flex-col divide-y divide-gray-700">
				{wishlist.map(items => <SingleWishlishItem key={items.id} items={items}></SingleWishlishItem>)
